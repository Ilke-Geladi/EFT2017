...
# Introduction
Tropical rainforests are the most biodiverse ecosystems in the world, housing two to three thirds of the species in the world [@Laurance2007; @Noss1999; @Bierregaard1992]. 
Reliable information on the status and condition in each forest studied, including the diversity of trees, is crucial to expanding our knowledge in fields such as forest dynamics, community ecology, and biotic interactions to ensure the best possible management and conservation strategies [@Noss1999; @Condit1995]. 
...
###BCI
For the first study site, we used tree diversity data from Barro Colorado Island (BCI) [@croat1978flora], which can be found in the package "vegan" [@oksanen2010vegan].
...
###Paracou
For the second study site, we used tree diversity data from Paracou [@Degen2006], which can be found in the package "entropart" [@Marcon2014c]. 
...
Through this method, we simulate the decrease in the size of the large community, in this case Paracou, to reduce it to the size of the small community, BCI [@marcon:cel-01205813]. 
...
Asymptotic diversity profiles reveal Paracou to be more diverse than BCI \@ref(fig:dp). 
As can be seen on \@ref(fig:dp), the Paracou diversity profile curve is above the BCI curve for all values of q.
...
```{r dp, echo=FALSE, fig.cap="Diversity Profiles for Paracou (top line) and BCI (bottom line). Number of simulations was set to 10 in the interest of saving time.", results='hide'}
...
It can further be said that BCI and Paracou are separable as the communities diversity profiles do not intersect [@marcon:cel-01205813]. 
There has been much debate as to which diversity indices to use as there exist many.
However, diversity profiles play an important role in converging these indices by portraying them graphically as the value of diversity is plotted in function to the order q, which represent different indices [@Tothmeresz1995; @marcon:cel-01205813]. 
